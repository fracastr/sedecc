...
$this->breadcrumbs=array(
	'Eval Instalaciones'=>array('index'),
	$model->eva_id,
);
?>
...
<h1>Detalle EvalInstalaciones #<?php echo $model->eva_id; ?></h1>
...
<?php
$pdf_safco = Yii::app()->db->createCommand("SELECT eess_rut FROM min_evaluacion_instalaciones WHERE eva_id = '".$model->eva_id."'")->queryScalar();
...
<div class="row">
	<div class="col-sm-4">
        ...
        array(
          ...
          'type'=>'raw',
          'value'=>function($data){
            return Yii::app()->db->createCommand("SELECT eess_rut FROM min_eess WHERE eess_rut = '".$data->eess_rut."'")->queryScalar();
          },
        ),
        array(
					'name'=>'Nombre EESS',
        ...
        'eva_apr',
        array(
          'name'=> 'Ejecutor Evaluación',
          'type'=>'raw',
          'value'=>function($data){
            return Yii::app()->db->createCommand("SELECT tra_nombres,tra_apellidos FROM min_trabajador where tra_rut = '".$data->eva_evaluador."'")->queryScalar();
	...
	<div class="col-sm-4">
        ...
        array(
          ...
          'value'=>function($data){
            ...
            return substr($data->eva_fecha_evaluacion,0,10);
            },
        ),
        array(
          'name'=>'Hora',
          ...
          'value'=>function($data){
            ...
            return substr($data->eva_fecha_evaluacion,11,5);
            },
        ),
        ...
        array(
          ...
          'type'=>'raw',
          'value'=>function($data){
            $georeferencia = "".$data->eva_geo_x.",".$data->eva_geo_y."";
	...
	<div class="col-sm-4">
		...
		else echo '<iframe frameborder="0" style="width:100%; height:261px;" src="https://static.parastorage.com/services/santa/1.1433.24/static/external/googleMap.html?addressInfo='.$model->eva_cod_faena.'%0A&language=es&lat='.$model->eva_geo_x.'&long='.$model->eva_geo_y.'&mapInteractive=true&mapType=ROADMAP&showMapType=true&showPosition=true&showStreetView=true&showZoom=true&ts=640&zoom=12"></iframe>';
		?>
	</div>
	<div class="col-sm-12">
		<?php
		$sql = "SELECT * FROM min_respuesta_instalaciones WHERE eva_id = '".$model->eva_id."' ORDER BY pre_id";
		...
		for($i=0,$j=1;$i<count($rows);$i++,$j++){
			...
			if($rows[$i]['res_respuesta'] == 'no'){
				$rows[$i]['res_enunciado'] = '<a href="index.php?r=respuestaInstalaciones/view&id='.$rows[$i]['res_id'].'"><b>'.$rows[$i]['res_enunciado'].'</b></a>';
			}
		...
		$ver= Yii::app()->db->createCommand("SELECT * FROM min_respuesta_instalaciones WHERE eva_id = '".$model->eva_id."' ORDER BY pre_id")->query()->readAll();
			...
			$critico = Yii::app()->db->createCommand("SELECT critico,pre_enunciado,res_enunciado, r.eva_id,res_respuesta,r.car_id,p.car_id FROM min_pregunta p inner join min_respuesta_instalaciones r on (pre_enunciado=res_enunciado) where res_enunciado = '".$enun."' AND r.eva_id = '".$model->eva_id."'")->query()->readAll();
		...
		$verpromedio = Yii::app()->db->createCommand("SELECT eva_cache_porcentaje From min_evaluacion_instalaciones WHERE eva_id='".$model->eva_id."'")->query()->readAll();
		...
		for($t=0;$t<count($verpromedio);$t++){
			...
			if($notapromedio>$limit1 and $tienecritico=='si'){
				...
				$alerta1 = ' ¡ALERTA! Instalación en Nivel de Riesgo ALTO';//';
