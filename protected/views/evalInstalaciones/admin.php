...
$('.search-form form').submit(function(){
	$('#evaluacion-instalaciones-grid').yiiGridView('update', {
		data: $(this).serialize()
	});
	return false;
});
...
<h1>Evaluacion Instalaciones</h1>
...
$this->widget('zii.widgets.grid.CGridView', array(
	'id'=>'evaluacion-instalaciones-grid',
	...
	'columns'=>array(
		...
		array(
			...
			'value'=>function($data){
				...
				$nos = Yii::app()->db->createCommand("SELECT COUNT(*) FROM min_respuesta_instalaciones WHERE eva_id = '".$data->eva_id."' AND res_respuesta = 'no' AND res_seguimiento = 0")->queryScalar();
				$ven = Yii::app()->db->createCommand("SELECT COUNT(*) FROM min_respuesta_instalaciones WHERE eva_id = '".$data->eva_id."' AND res_plazo < NOW() AND res_seguimiento = 0")->queryScalar();
				$s='';
				if($nos > 0){
					if($ven > 0){
				...
				Yii::app()->db->createCommand("UPDATE min_evaluacion_instalaciones SET eva_semaforo = '".$s."' WHERE eva_id = '".$data->eva_id."'")->execute();
				// Retornar
				return '<center>'.$semaforo.'</center>';
			},
		),
