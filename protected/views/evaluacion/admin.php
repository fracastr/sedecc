	...
	'columns'=>array(
		...
		array(
			...
			'value'=>function($data){
				...
				$nos = Yii::app()->db->createCommand("SELECT COUNT(*) FROM min_respuesta WHERE eva_id = '".$data->eva_id."' AND res_respuesta = 'no' AND res_seguimiento = 0")->queryScalar();
				$ven = Yii::app()->db->createCommand("SELECT COUNT(*) FROM min_respuesta WHERE eva_id = '".$data->eva_id."' AND res_plazo < NOW() AND res_seguimiento = 0")->queryScalar();
				$s='';
				if($nos > 0){
					if($ven > 0){
