...
<div class="row">
	...
	<div class="col-sm-12">
		<?php
		$sql = "SELECT * FROM min_respuesta WHERE eva_id = '".$model->eva_id."' ORDER BY pre_id";
		...
		$ver= Yii::app()->db->createCommand("SELECT * FROM min_respuesta WHERE eva_id = '".$model->eva_id."' ORDER BY pre_id")->query()->readAll();
			...
			$critico = Yii::app()->db->createCommand("SELECT critico,pre_enunciado,res_enunciado, r.eva_id,res_respuesta,r.car_id,p.car_id FROM min_pregunta p inner join min_respuesta r on (pre_enunciado=res_enunciado) where res_enunciado = '".$enun."' AND r.eva_id = '".$eval_id."' ")->query()->readAll();
		...
		$verpromedio = Yii::app()->db->createCommand("SELECT eva_cache_porcentaje From min_evaluacion WHERE eva_id=".$eval_id)->query()->readAll();
