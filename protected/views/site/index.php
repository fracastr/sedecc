...
setTimeout("window.location='?eess="+opc+"'",t*1000); 
} 
</script> 
  ...
  <div class="col-md-2">
    <div align="center" id="chartdiv0" style="width: 100%; height: 133px; background-color: white;">
      <div style="width: 100%; margin-bottom: 5px; border-bottom: 2px solid #eeecee;">N° de Trabajadores</div>
            <?php
            $rows = Yii::app()->db->createCommand("SELECT COUNT(*) as trab FROM min_trabajador as E JOIN min_eess as EE ON(E.eess_rut = EE.eess_rut) WHERE EE.eess_estado = 1 ".$where2." ".$where." ")->query()->readAll();
      ...
      <div style="width: 100%; margin-bottom: 5px; margin-top: 10px; border-bottom: 2px solid #eeecee;">N° de Evaluaciones</div>
            <?php
            $rows = Yii::app()->db->createCommand("SELECT COUNT(*) as trab FROM min_evaluacion as E JOIN min_eess as EE ON(E.eess_rut = EE.eess_rut) WHERE EE.eess_estado = 1 ".$where2." ".$where." ")->query()->readAll();
...
<div class="row" style="margin-top: 10px;">
  <div class="col-md-12">
    ...
    <div class="col-md-12" align="center" style="background-color: #FFFFFF;">
      <div style="border-bottom: 2px solid #eeecee;">% Cumplimiento por Jefe de Faena</div>
...
<script type="text/javascript">
          ...
          "graphs": [
    {
      ...
      "balloonText": "[[Porcentaje]]%",
          ...
          "dataProvider": [
            <?php
            $rows = Yii::app()->db->createCommand("SELECT eva_jefe_faena, AVG(eva_cache_porcentaje) as porc FROM min_evaluacion as E JOIN min_eess as EE ON(E.eess_rut = EE.eess_rut) WHERE EE.eess_estado = 1 and eva_jefe_faena != '' ".$where2." ".$where." GROUP BY eva_jefe_faena")->query()->readAll();
            for($i=0;$i<count($rows);$i++){
              $rows[$i]['color'] = '#e14d57';
              if($rows[$i]['porc'] > Yii::app()->params['riesgoalto']) $rows[$i]['color'] = '#f7e523';
...
<script type="text/javascript">
      AmCharts.makeChart("chartdiv5",
        ...
        "lineColorField": "lineColor"
          ...
          "dataProvider": [
            <?php
            $rows = Yii::app()->db->createCommand("SELECT YEAR(eva_fecha_evaluacion) as ano, MONTH(eva_fecha_evaluacion) as mes, AVG(eva_cache_porcentaje) as prom FROM min_evaluacion as E JOIN min_eess as EE ON(E.eess_rut = EE.eess_rut) WHERE EE.eess_estado = 1 ".$where2." ".$where." GROUP BY YEAR(eva_fecha_evaluacion), MONTH(eva_fecha_evaluacion) ORDER BY eva_fecha_evaluacion ASC LIMIT 0,6")->query()->readAll();
            for($i=0;$i<count($rows);$i++){
              $mes='';
              if($rows[$i]['mes'] == 1){
                $mes = 'Enero';
      ...
      var chart2 = AmCharts.makeChart("chartdiv22",
        {
          "type": "pie",
          "dataLoader": {
              "url": "graphsC/corma2.php?eess=<?php echo $u; ?>&empresa=<?php echo $eess; ?>"
          },
      ...
      var chart3 = AmCharts.makeChart("chartdiv33",
        {
          "type": "pie",
          "dataLoader": {
              "url": "graphsC/ocupacional2.php?eess=<?php echo $u; ?>&empresa=<?php echo $eess; ?>"
          },
          "percentPrecision": 0,
          "balloonText": "[[title]]<br><span style='color: black; font-size:14px'><b>[[value]]</b> ([[percents]]%)</span>",
          "startDuration": 1,
      ...
      var chart4 = AmCharts.makeChart("chartdiv44",
        {
          "type": "pie",
          "dataLoader": {
              "url": "graphsC/licencia2.php?eess=<?php echo $u; ?>&empresa=<?php echo $eess; ?>"
          },
          "percentPrecision": 0,
          "balloonText": "[[title]]<br><span style='color: black; font-size:14px'><b>[[value]]</b> ([[percents]]%)</span>",
          "startDuration": 1,
...
var chart = AmCharts.makeChart("chartdiv99", {
  "type": "serial",
  "dataLoader": {
              "url": "graphsC/tiempo2.php?eess=<?php echo $u; ?>&empresa=<?php echo $eess; ?>"
          },
  "theme": "light",
  "dataDateFormat": "YYYY-MM-DD",
  "startDuration": 1,
      ...
      var chart66 = AmCharts.makeChart("chartdiv66",
        {
          "type": "pie",
          "dataLoader": {
              "url": "graphsC/desviacion.php?eess=<?php echo $u; ?>&empresa=<?php echo $eess; ?>"
          },
...
FROM(SELECT AVG(eva_cache_porcentaje) as porc FROM min_evaluacion as E JOIN min_eess as EE ON(E.eess_rut = EE.eess_rut) WHERE EE.eess_estado = 1  ".$where2." ".$where.") as czx
                    ";
