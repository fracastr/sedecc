  ...
  <div class="col-md-2">
    <div align="center" id="chartdiv0" style="width: 100%; height: 135px; background-color: white;">
      <div style="width: 100%; margin-bottom: 5px; border-bottom: 2px solid #eeecee;">N° de Trabajadores</div>
            <?php
            $rows = Yii::app()->db->createCommand("SELECT COUNT(*) as trab FROM min_trabajador WHERE 1 ".$where." ")->query()->readAll();
      ...
      <div style="width: 100%; margin-bottom: 5px; margin-top: 10px; border-bottom: 2px solid #eeecee;">N° de Evaluaciones</div>
            <?php
            $rows = Yii::app()->db->createCommand("SELECT COUNT(*) as trab FROM min_evaluacion WHERE 1 ".$where." ")->query()->readAll();
...
<script type="text/javascript">
      AmCharts.makeChart("chartdiv",
        {
          ...
          <?php
          $porc = Yii::app()->db->createCommand("SELECT AVG(eva_cache_porcentaje) FROM min_evaluacion WHERE 1 ".$where." ")->queryScalar();
          $nota = '';
          ...
          "graphs": [
    {
      "balloonText": "[[Porcentaje]]%",
          ...
          "dataProvider": [
            <?php
            $rows = Yii::app()->db->createCommand("SELECT eva_faena, AVG(eva_cache_porcentaje) as porc FROM `min_evaluacion` WHERE eva_faena != '' ".$where." GROUP BY eva_faena")->query()->readAll();
            for($i=0;$i<count($rows);$i++){
              $rows[$i]['color'] = 'red';
              if($rows[$i]['porc'] > Yii::app()->params['limit1']) $rows[$i]['color'] = 'yellow';
              if($rows[$i]['porc'] > Yii::app()->params['limit2']) $rows[$i]['color'] = 'green';
...
<script type="text/javascript">
      AmCharts.makeChart("chartdiv5",
        ...
        "lineColorField": "lineColor"
          ...
          "dataProvider": [
            <?php
            $rows = Yii::app()->db->createCommand("SELECT YEAR(eva_fecha_evaluacion) as ano, MONTH(eva_fecha_evaluacion) as mes, AVG(eva_cache_porcentaje) as prom FROM min_evaluacion WHERE 1 ".$where." GROUP BY YEAR(eva_fecha_evaluacion), MONTH(eva_fecha_evaluacion) ORDER BY eva_fecha_evaluacion ASC LIMIT 0,6")->query()->readAll();
            for($i=0;$i<count($rows);$i++){
              $mes='';
              if($rows[$i]['mes'] == 1){
                $mes = 'Enero';
      ...
      var chart2 = AmCharts.makeChart("chartdiv22",
        {
          "type": "pie",
          "dataLoader": {
              "url": "graphsC/corma.php?eess=<?php echo $u; ?>"
          },
          "percentPrecision": 0,
      ...
      var chart3 = AmCharts.makeChart("chartdiv33",
        {
          "type": "pie",
          "dataLoader": {
              "url": "graphsC/ocupacional.php?eess=<?php echo $u; ?>"
          },
          "percentPrecision": 0,
          "balloonText": "[[title]]<br><span style='color: black; font-size:14px'><b>[[value]]</b> ([[percents]]%)</span>",
          "startDuration": 0,
      ...
      var chart4 = AmCharts.makeChart("chartdiv44",
        {
          "type": "pie",
          "dataLoader": {
              "url": "graphsC/licencia.php?eess=<?php echo $u; ?>"
          },
          "percentPrecision": 0,
          "balloonText": "[[title]]<br><span style='color: black; font-size:14px'><b>[[value]]</b> ([[percents]]%)</span>",
          "startDuration": 0,
...
var chart = AmCharts.makeChart("chartdiv99", {
  "type": "serial",
  "dataLoader": {
              "url": "graphsC/tiempo.php?eess=<?php echo $u; ?>"
          },
  "theme": "light",
  "dataDateFormat": "YYYY-MM-DD",
  "fontSize": 7,
...
FROM(SELECT AVG(eva_cache_porcentaje) as porc FROM min_evaluacion WHERE 1 AND eess_rut = '".$u."') as czx
                    ";
