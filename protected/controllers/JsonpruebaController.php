...
class JsonController extends Controller
{
	...
	public function actionAutocompletar(){
		...
		//$global['patente'] = Yii::app()->db->createCommand("SELECT pat_rut_eess,pat_rut_trabajador,pat_patente as eva_patente FROM min_patentes")->query()->readAll();
		echo json_encode($global);
	...
	public function actionRecep(){
		...
		$sqlversion = "SELECT version_antigua,version_actual,version_nueva FROM min_control_version";
		...
		if(isset($_POST['control_version'])){
			//version actual
		...
		else if($_POST['control_version']==$nueva){
			if(isset($_POST['android'])){
				//Parche para variables no definidas, util para actualizaciones
				if(!isset($_POST['android'])) $_POST['android']='';
				if(!isset($_POST['timestamp'])) $_POST['timestamp']='';
				...
				if(!isset($_POST['eva_cargo'])) $_POST['eva_cargo']='';
				//vesionamiento
				if(!isset($_POST['control_version'])) $_POST['control_version']='';
				...
				$content = json_encode(
					array(
						...
						//versiones
						"control_version"=>utf8_encode($_POST['control_version']),
