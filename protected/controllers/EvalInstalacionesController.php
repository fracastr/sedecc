...
class EvalInstalacionesController extends Controller
{
	/**
	...
	SUBSTRING(eva_jefe_faena,LOCATE(' ',eva_jefe_faena,LOCATE(' ',eva_jefe_faena)+1),LENGTH(eva_jefe_faena)) as apellido_jefe_faena FROM min_evaluacion_instalaciones WHERE eva_id = ".$id)->queryRow();
	$fechaeva =  $evaluacion['eva_fecha_evaluacion'];
	$fechacorta = substr($fechaeva,0,10);
	//$fechacorta = $evaluacion['eva_fecha_evaluacion'];
	$hora = substr($fechaeva,10,16);
	//$nombre_ejecutor = Yii::app()->db->createCommand("SELECT tra_nombres,tra_apellidos FROM min_trabajador where tra_rut = '".$evaluacion['eva_evaluador']."'")->queryScalar();
	$correlativo = Yii::app()->controller->identificador($evaluacion['eva_evaluador'],$evaluacion['eva_fecha_evaluacion'],$evaluacion['eva_evaluador_correlativo']);
		...
		<tr>
			<td>
				...
				<tr>
					...
					<td width="218">
						: '.$evaluacion['eva_gerente_general'].'
					</td>
					<td class="th-sortable" data-toggle="class" style="text-align: left; width:115;">
						Código Faena
					</td>
					<td width="230">
						: '.$evaluacion['eva_cod_faena'].'
					</td>
				</tr>
				<tr>
					<td class="th-sortable" data-toggle="class" style="text-align: left; width:115;">
					...
					<td width="218">
						: '.$evaluacion['eva_gerente_operacion'].'
					</td>
					<td class="th-sortable" data-toggle="class" style="text-align: left; width:115;">
						Nº Trabajadores:
					</td>
					<td width="230">
						: '.$evaluacion['eva_num_trabajadores'].'
					</td>
				</tr>
				<tr>
					<td class="th-sortable" data-toggle="class" style="text-align: left; width:115;">
		...
		';
		$tematicas = Yii::app()->db->createCommand("SELECT DISTINCT tem_id FROM min_respuesta_instalaciones WHERE eva_id = ".$id)->query()->readAll();
		for($i=0;$i<count($tematicas);$i++){
			...
			<tr>
				<td>
					...
					';
					$preguntas = Yii::app()->db->createCommand("SELECT * FROM min_respuesta_instalaciones WHERE tem_id = '".$tematicas[$i]['tem_id']."' AND eva_id = ".$id)->query()->readAll();
		...
		for($s=0;$s<count($ver);$s++){
				...
				if($ver[$s]['res_critico'] == 'si'&&$ver[$s]['res_respuesta']=='no'){
					$alerta1 = '¡Precaución!: Instalación ha incumplido una pregunta considerada crítica.';//';
		...
		for($t=0;$t<count($verpromedio);$t++){
			...
			if($notapromedio>85 and $tienecriticosi=='si'){
				...
				$alerta1 = ' ¡ALERTA! Instalacion en Nivel de Riesgo ALTO';//';
				$alerta2 = ' Se recomienda detener los trabajos y reorganizar Faena.';//';
		...
		';
		$resp2 = Yii::app()->db->createCommand("SELECT count(*) as contObs FROM min_respuesta_instalaciones where (res_foto != '' or res_observacion != '') AND eva_id = ".$id)->query()->readAll();
		...
		';
			...
			$resp = Yii::app()->db->createCommand("SELECT * FROM min_respuesta_instalaciones where (res_foto != '' or res_observacion != '') AND eva_id = ".$id)->query()->readAll();
	 ...
	 */
 	...
 	{
 		$model=new EvalInstalaciones;

 		// Uncomment the following line if AJAX validation is needed
 		// $this->performAjaxValidation($model);

 		if(isset($_POST['EvalInstalaciones']))
 		{
 			$model->attributes=$_POST['EvalInstalaciones'];
 			if($model->save())
 		...
 		if(isset($_POST['EvalInstalaciones']))
 		{
 			...
 			if($model->save()){
 				...
 				foreach ($preguntas as $key => $value){
 					// Subir imagen
 					$dir_subida = 'imagenes_respuestas_instalaciones/';
 					...
 					UPDATE min_respuesta_instalaciones
 				...
 				// Actualizar porcentaje cache
 				$sql="UPDATE min_evaluacion_instalaciones SET eva_cache_porcentaje = '".$nota."' WHERE eva_id = ".$id;
 	...
 	{
 		$model=new EvalInstalaciones('search');
 		$model->unsetAttributes();  // clear any default values
 		if(isset($_GET['EvalInstalaciones']))
 			$model->attributes=$_GET['EvalInstalaciones'];

 		$this->render('admin',array(
 			'model'=>$model,
 		));
 		/*
 		$dataProvider=new CActiveDataProvider('EvalInstalaciones');
 	 ...
 	 * @return EvalInstalaciones the loaded model
 	 * @throws CHttpException
 	 */
 	public function loadModel($id)
 	{
 		$model=EvalInstalaciones::model()->findByPk($id);
 		if($model===null)
 	 ...
 	 * @param EvalInstalaciones $model the model to be validated
 	 */
 	protected function performAjaxValidation($model)
 	{
 		if(isset($_POST['ajax']) && $_POST['ajax']==='eval-instalaciones-form')
 		{
